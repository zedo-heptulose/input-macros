...
class ORCAInputBuilder:
    ...
    def build(self):
        ...
        newjob.inp.strings.append(f"%maxcore {int(maxcore)}")
        if not self.config['blocks'].get('pal',None):
            newjob.inp.blocks['pal'] = [f"nprocs {self.config['num_cores']}",]
            
        for name in self.config['blocks']:
            newjob.inp.blocks['name'] = self.config['blocks'][name]
        
        if self.config['broken_symmetry']:
            if not newjob.inp.blocks.get('scf',None):
                newjob.inp.blocks['scf'] = ['brokensym 1,1']
            else:
                condition = False
                for line in newjob.inp.blocks['scf']:
                    if 'brokensym' in line.lower():
                        condition = True
                if not condition:
                    newjob.inp.blocks['scf'].append('brokensym 1,1')
        
        newjob.inp.charge = self.config['charge']
        newjob.inp.multiplicity = self.config['spin_multiplicity']
        newjob.inp.xyzfile = self.config['xyz_file']
